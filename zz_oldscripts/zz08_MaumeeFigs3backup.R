...
# #alternative
# Mall75_95 <- Mall %>% 
#   filter(Y >= "1975" & Y <= "2003") 
# 
# # Determine SS load v Q relationship from 1975 to 1995
# # 1987 is weird
# ggplot(Mall75_95, aes(y = log(gSSmarJun_50per), x = log(Qm3marJun_50per), label = Y)) +
#   geom_text() +
#   stat_smooth(method = "lm")
# 
# SSvQ_7595_lm <- lm(log(gSSmarJun_50per) ~ log(Qm3marJun_50per), Mall75_95)
# summary(SSvQ_7595_lm)
# 
# MallF4a <- Mall %>% 
#   # estimate the SS load using the SS load v Q relationship from 1975 to 1995
#   mutate(gSSmarJun_50per_7595 = exp(coefficients(SSvQ_7595_lm)[[1]] + log(Qm3marJun_50per) * coefficients(SSvQ_7595_lm)[[2]]),
#          # estimate the P sorption with that SS load
#          gPsorbed_50per_7595ss = gSSmarJun_50per_7595 * gPsorbedgDM,
#          # estimate what DRP load would be witm gPsorbed_50per_7595ss
#          gDRPestW7595ss = (gPsorbMarJunP_50per + gDRPmarJun_50per) - gPsorbed_50per_7595ss)
# 
# ggplot(MallF4a %>% 
#          filter(Y <= 1995), aes(y = gSSmarJun_50per_7595, x = gSSmarJun_50per, label = Y, color = Yg)) +
#   geom_text()+
#   geom_abline(intercept = 0, slope = 1)
# 
# ggplot(MallF4a, aes(y = gSSmarJun_50per_7595, x = gSSmarJun_50per, label = Y, color = Yg)) +
#   geom_text()+
#   geom_abline(intercept = 0, slope = 1)
# 
# ggplot(MallF4a, aes(y = gPsorbed_50per_7595ss, x = gPsorbMarJunP_50per, label = Y, color = Yg)) +
#   geom_text()+
#   geom_abline(intercept = 0, slope = 1)
# 
# ggplot(MallF4a, aes(y = gDRPestW7595ss, x = gDRPmarJun_50per, label = Y, color = Yg)) +
#   geom_text()+
#   geom_abline(intercept = 0, slope = 1)
...
Fig4A <- ggplot() +
  ...
  geom_ribbon(data = MallF4a %>% 
                filter(Y > 2003), aes(ymin = gDRPestW7595ss/1e6, ymax = gDRPmarJun_50per/1e6, x = Y), fill = "steelblue1", alpha = 40/100) +
  geom_segment(data = Mall, aes(y = gDRPmarJun_50per/1e6 + gPsorbMarJunP_50per/1e6, yend =  gDRPmarJun_50per/1e6+5, x = Y, xend=Y), 
               arrow = arrow(length = unit(0.1, "inches")), color = "steelblue4", size = 1.15)+
  geom_segment(data = MallF4a %>% 
                 filter(Y > 2003), aes(y = gDRPmarJun_50per/1e6, yend =  gDRPestW7595ss/1e6 +5, x = Y, xend=Y),
               arrow = arrow(length = unit(0.1, "inches")), color = "steelblue1", size = 1.15) +
  geom_point(data = Mall, aes(y = gDRPmarJun_50per/1e6, x = Y), fill = "steelblue4", shape = 21, size = 3) +
  geom_point(data = Mall, aes(y = gDRPmarJun_50per/1e6 + gPsorbMarJunP_50per/1e6, x = Y), fill = "red", shape = 21, size = 3) +
  geom_point(data = MallF4a %>% 
               filter(Y > 2003), aes(y =  gDRPestW7595ss/1e6, x = Y), fill = "steelblue1", shape = 21, size = 3) +
  ...
  geom_point(aes(y =620, x = 1975), color = "red", size = 5) +
  annotate("text", label = "Without P sorption", y = 620, x = 1977, size = 8, hjust = 0) +
  geom_point(aes(y =585, x = 1975), color = "steelblue4", size = 5) +
  annotate("text", label = "Observed", y = 585, x = 1977, size = 8, hjust = 0) +
  geom_point(aes(y =545, x = 1975), color = "steelblue1", size = 5) +
  annotate("text", label = "With 1975-2003 sediment loads", y = 545, x = 1977, size = 8, hjust = 0) 
...
# increase in DRP MODELS
DRPvQintYn.l1 <- lm(L10.mtDRPmarJun_50per ~ L10.Qm3e6_marJun_50per * Yn, Mall2); summary(DRPvQintYn.l1)
DRPvQintYn.l2 <- lm(L10.mtDRPmarJun_50per ~ L10.Qm3e6_marJun_50per + Yn, Mall2); summary(DRPvQintYn.l2)
DRPvQintYn.l3 <- lm(L10.mtDRPmarJun_50per ~ L10.Qm3e6_marJun_50per, Mall2); summary(DRPvQintYn.l3)
AICc(DRPvQintYn.l1, DRPvQintYn.l2, DRPvQintYn.l3) # lowest AICc is additive model
...
SSload1999 <- 10^predict(SSvQpYn.l, as.data.frame(as.matrix(cbind(L10.Qm3e6_marJun_50per = log10(TargetQ), Yn = 1999))))
SSload2019 <- 10^predict(SSvQpYn.l, as.data.frame(as.matrix(cbind(L10.Qm3e6_marJun_50per = log10(2420), Yn = 2019))))
DRPload1999 <- 10^predict(DRPvQintYn.l2, as.data.frame(as.matrix(cbind(L10.Qm3e6_marJun_50per = log10(2420), Yn = 1999))))
DRPload2019 <- 10^predict(DRPvQintYn.l2, as.data.frame(as.matrix(cbind(L10.Qm3e6_marJun_50per = log10(2420), Yn = 2019))))
...
# DRP
PerDeclineDRPDecade <- (DRPload2019 -DRPload1999)/mean(c(DRPload2019,DRPload1999))/2
AbsDeclineDRPDecade <- (DRPload2019 -DRPload1999)/2

# decline in P sorption from 2003/2019
mtPsorb1999 <- SSload1999*quantile(Mall2$gPsorbedgDM, probs = 0.5)
mtPsorb2019 <- SSload2019*quantile(Mall2$gPsorbedgDM, probs = 0.5)
...
Fig4B.g <- ggplotGrob(Fig4D)
Fig4C.g <- ggplotGrob(Fig4C)


Fig4A.gtf <- gtable_frame(Fig4A.g,  width = unit(1.8, "null"), height = unit(0.9, "null"))
Fig4B.gtf <- gtable_frame(Fig4C.g,  width = unit(0.9, "null"), height = unit(0.9, "null"))
Fig4C.gtf <- gtable_frame(Fig4D.g,  width = unit(0.9, "null"), height = unit(0.9, "null"))
...
Fig4BC.gtf <- gtable_frame(gtable_cbind(Fig4B.gtf, Fig4C.gtf),  width = unit(0.9, "null"), height = unit(0.9, "null"))
...
cairo_pdf("05_Figures/08_Fig4_2.pdf", height = 12, width = 15)
grid.newpage()
grid.draw(Fig4.gtf)
...
grid.text("c", x = unit(0.49,"npc"), y = unit(0.5,"npc"), gp=gpar(fontsize = 25, fontface = "bold"))
dev.off()
...
dev.off()
  ...
  # SAVE/LOAD
  ################
  # DON'T USE SAVED IMAGE UNTIL RERUN EVERYTHING
  # save.image("03_Rdata/08_MaumeeFigs_Rdat")
  load("03_Rdata/08_MaumeeFigs_Rdat")
  
  # change in DRP loads
    Mall_03on <- Mall %>% 
        filter(Y >= 1990)

    ggplot(Mall_03on, aes(y = gDRPmarJun_50per, x = Y)) +
      geom_point()
