# JMH Feb 2021
# Makes Fig S10
# Also does analyses for Table S3
...
# takes a couple mins
MaumBSraw <- read.csv(file.path(here::here("04a_generatedDataTooBigForGit"), "04b_RawBootstrapResults_Maumee_50perDRP.csv"))[-1,-1]
...
## DRP~Q with observed loads ----
  ...
  MR2_L03$mtDRP_MjHf_pred_se <- (predict(DRP_Q_obs_glm_int, MR2_L03, se.fit = T)$se.fit)
  
  MR2_G03 <- MR2 %>% 
    filter(Y >= 2003)
  MR2_G03$mtDRP_MjHf_pred <- ( predict(DRP_Q_obs_glm_int, MR2_G03))
  MR2_G03$mtDRP_MjHf_pred_se <- ( predict(DRP_Q_obs_glm_int, MR2_G03, se.fit = T)$se.fit)
  MR2_G03$mtDRP_MjHf_pred75ss <- (predict(DRP_Q_obs_glm_75loads_add, MR2_G03))
  MR2_G03$mtDRP_MjHf_pred75ss_se <- (predict(DRP_Q_obs_glm_75loads_add, MR2_G03, se.fit = T)$se.fit)
  ...
  FigS9.g <- ggplotGrob(FigS9)
  FigS9.gtf <- gtable_frame(FigS9.g,  width = unit(0.8, "null"), height = unit(0.9, "null"))
  
  png("05_Figures/11c_FigS10.png", units = "in", height = 7, width = 7, res = 300)
  grid.newpage()
  grid.draw(FigS9.gtf)
  dev.off()

# save ----
  # write.csv(MR2_G03, file.path(here::here("04a_generatedDataOnGit"), "11a_Fig4dat_50perDRP_G03.csv"))
  # write.csv(MR2_L03, file.path(here::here("04a_generatedDataOnGit"), "11a_Fig4dat_50perDRP_L03.csv"))
  
  # save.image(file.path(here::here("03_Rdata"), "11c_FigS10_TableS3_50perDRP_Rdat"))
  # load(file.path(here::here("03_Rdata"), "11c_FigS10_TableS3_50perDRP_Rdat"))
