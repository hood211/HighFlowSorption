# JMH; Fall 2020
# Downloads Q data for South Turkey foot (STF)
...
Turk <- qTurk %>% 
  ...
  select(-agency_cd, -dateTime.y, -TP_mgL, -c(NO23_mgL:Silica_mgL)) %>% 
  ...
  arrange(dateTime) %>% 
  # replacing zeros with minimum non-zero value
  # since no info on detection limit in docs
  mutate(SS_mgL = ifelse(SS_mgL <= 0, 0.6812, SS_mgL),
         SRP_mgL = ifelse(SRP_mgL <= 0, 0.001, SRP_mgL)) %>% 
...
# Also tried models with month, but they were not as good
...
# Low flows  ----
# this makes calcs for low flows, 
...
# Export ----
write.csv(Turkfin, "04a_generatedDataOnGit/02f_Turk.csv")
save.image("03_Rdata/02_Turk_Rdat")
load("03_Rdata/02_Turk_Rdat")
