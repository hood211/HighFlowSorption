...
streamWQ <- read.csv("04_generatedData/02f_Turk.csv")
...
#                        TimeWin, SS_gDMm3, tt_pref, StreamDRP){
  ...
  ###################### 
  #scale for reach one between STF to  Maumee @ WAterville
  #####################
  # reach parameters
  StreamWA1 <- as.numeric(gsWA[gsWA$USGS.gauge == "South Turkeyfook Ck near Shunk",]$WA_m2)
  slope_mm1 <- segInfo[segInfo$Gauge_start == "STF",]$SlopeMm
  seglg_m1 <- segInfo[segInfo$Gauge_start == "STF",]$SegLgM
  ...
  ManningsCoef <- 0.037/60 # from Barnes report (units are metric s/m^(1/3) so converting to min)
  StreamQan1 <- MeanAnnQS[MeanAnnQS$stream == "Qm3s",]$Q_m3s*60
  ...
  # run function
  streamWQatMRWAT <- ScalingFun.step1(WQdat = streamWQ2,
                                      Qm3m = Qm3m, 
                                      SSgDMm3 = SSgDMm3, 
                                      DRPgPm3 = DRPgPm3, 
                                      slope_mm = segInfo[segInfo$Gauge_start == "STF",]$SlopeMm, 
  ...
  # Almost always reaching capacity, but no place to go
  ggplot(streamWQatMRWAT %>% 
  ...
  streamWQatMRWAT.stfHF <- streamWQatMRWAT %>% 
  ...
  STF_PercentSorbStmp = 1-(streamWQatMRWAT.stfHF$DRPloadDS.gPwin.wS.hf_sum/ streamWQatMRWAT.stfHF$DRPloadUS.gPwin.hf_sum)
  
  #####################
  # BOOTSTRAP
  #####################
  BootNum <- 500
  streamWQ_ibs <- as.data.frame(matrix(nrow = 1, ncol = dim(streamWQatMRWAT)[2])) %>% 
    mutate(
          # Sms_gPgDMmin.Med = as.numeric("NA"),
          #  Scap_gPgDM = as.numeric("NA"),
          #  StreamEPC = as.numeric("NA"),
           bootN = as.numeric("NA"))
  names(streamWQ_ibs) <- c(names(streamWQatMRWAT), "bootN") #"Sms_gPgDMmin.Med", "Scap_gPgDM", "StreamEPC",
  ...
  for(i in 1:BootNum) {
    ...
    Sms_gPgDMmin.Med_i = SorDatSN$Sms_gPgDMmin[sample(SorpSmpN, size = dim(streamWQatMRWAT)[1], replace = TRUE)]
    Scap_gPgDM_i = SorDatSN$Scap_gPgDM[sample(SorpSmpN, size = dim(streamWQatMRWAT)[1], replace = TRUE)] 
    StreamEPC_i = SorDatSN$EPC_gPm3[sample(SorpSmpN, size =dim(streamWQatMRWAT)[1], replace = TRUE)]
  ...
  streamWQatMRWATbs.Sdoy <- streamWQ_ibs %>% 
    mutate(DRPloadUS.gPwin.hf = ifelse(gTarPerFlow == "Gtarget", DRPloadUS.gPwin, 0),
  ...
  StartStumpf <- as.POSIXct("2019-03-01", format = "%Y-%m-%d")
  EndStumpf <- as.POSIXct("2019-07-01", format = "%Y-%m-%d")
  streamWQatMRWAT.stfHF <- streamWQ_ibs %>% 
    # subset to stumpf window
    filter(dateTime >= StartStumpf & dateTime <= EndStumpf) %>% 
    mutate(DRPloadUS.gPwin.hf = ifelse(gTarPerFlow == "Gtarget", DRPloadUS.gPwin, 0),
           Svol_gPwindow.hf = ifelse(gTarPerFlow == "Gtarget", Svol_gPwindow, 0)) %>% 
    select(dateTime, DRPloadUS.gPwin, DRPloadUS.gPwin.hf, Svol_gPwindow.hf, Qm3m, bootN) %>% 
    group_by(bootN) %>% 
  ...
  write.csv(streamWQ_ibs, "04_generatedData/05d_SFTboot.csv")
  
  # save image
  save.image("03_Rdata/05z_scaleSTF_rdat")
