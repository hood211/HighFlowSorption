...
maum <- read.csv(file.path(here::here("04a_generatedDataOnGit"), "04_RawBootstrapResults_Maumee.csv"))[-1,-1]
...
wqMaumCyanos <- maumMjHf2 %>%
  ...
  pivot_longer(cols = c(mtDRP_MjAllF:cyano_modelMean_woSorp), names_to = "var", values_to = "values")
    ...
    # only works if you don't run the last line of above
    # ggplot() +
    #   geom_point(data = wqMaumCyanos, aes(y = cyanos_Bert_obs, x = mtBioavaP_MjAllf_S)) +
    #   geom_point(data = wqMaumCyanos, aes(y = cyano_Stump_obs, x = mtBioavaP_MjAllf_S), color = "blue")
...
# Export data ----
# write.csv(maumMjHf2, file.path(here::here("04a_generatedDataOnGit"), "06_Scale2WVall_MjHf.csv"))
# write.csv(wqMaumCyanos, file.path(here::here("04a_generatedDataOnGit"), "06_Scale2WVall_MjHf_cyanos.csv"))

# Save image
# save.image(file.path(here::here("03_Rdata"), "06_MaumeeBootMungePcyanos_Rdat"))
# load(file.path(here::here("03_Rdata"), "06_MaumeeBootMungePcyanos_Rdat"))
