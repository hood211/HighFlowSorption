...
WC <- read.csv(file.path(here::here("04a_generatedDataTooBigForGit"), "04b_TribRawBootstrapResults_WC_50perDRP.csv"))[-1,] 
UTLC <- read.csv(file.path(here::here("04a_generatedDataTooBigForGit"), "04b_TribRawBootstrapResults_UTLC_50perDRP.csv"))[-1,] 
STF <- read.csv(file.path(here::here("04a_generatedDataTooBigForGit"), "04b_TribRawBootstrapResults_STF_50perDRP.csv"))[-1,] 
...
TribSorbS2 <- TribSorbS %>% 
  ...
  mutate(SorbDistPerWat = ifelse(stream == "STF", (SorbDistM_50per_m/38715.5)*100,
         ...
         # distance to waterville from trib gage (number in meters than converted to km)
...
# Patterns in P sorp ----
# used in results
TribSorbS3 <- TribSorbS2 %>% 
            mutate(Psorb = as.factor(ifelse(Svol_gPwindow.hf_50per_s > 0, "Sorb", "NotSorb")))
summary(TribSorbS3[TribSorbS3$stream == "UTLC",]$Psorb)
summary(TribSorbS3[TribSorbS3$stream == "WC",]$Psorb)
summary(TribSorbS3[TribSorbS3$stream == "STF",]$Psorb)


ggplot(TribSorbS2, aes(y = Svol_gPwindow.hf_50per_s, x = DOY)) +
  geom_point() +
...
# save.image("03_Rdata/05_TribFigs_50perDRP_rdat")
# load("03_Rdata/05_TribFigs_50perDRP_rdat")

# save data
# write.csv(TribSorbSsw,file.path(here::here("04a_generatedDataOnGit"), "05b_TribDat4Fig2a_50col.csv"))
# write.csv(TribSorbS2,file.path(here::here("04a_generatedDataOnGit"), "05b_TribDat4Fig2bc_50col.csv"))



# Fig 2 with colloid-P/DRP = 0.5 ----
...
Fig2b <- ggplot() +
  ...
  geom_point(data = TribSorbS2, aes(y = (Svol_gPwindow.hf_50per_s+0.075)/1000, x = DRPloadUS.gPwin.hf_50per_s/1000, fill = stream, size = Qm3m_50per_m),
             shape = 21, alpha = 0.5) +
  ...
  stat_smooth(data = TribSorbS2, aes(y = (Svol_gPwindow.hf_50per_s+0.075)/1000, x = DRPloadUS.gPwin.hf_50per_s/1000, color = stream), method = "lm")
...
## Print fig ----
png("05_Figures/05_Fig2_50perDRP.png", units="in", width=8, height=7, res=300)
grid.newpage()
grid.draw(Fig2.gtf)
